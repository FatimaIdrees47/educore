...
<div class="page-header">
    ...
    <p class="page-subtitle">Communicate with your child's teachers</p>
</div>
...
<div class="row g-3">
    ...
    <div class="col-md-4">
        <div class="educore-card" style="padding:0;overflow:hidden">
            ...
            @php
                ...
                $isActive = isset($teacher) && $teacher->id === $other->id;
            @endphp
            <a href="{{ route('parent.messages.show', $other->id) }}"
               style="display:block;text-decoration:none;
                      background:{{ $isActive ? 'rgba(219,39,119,0.06)' : 'transparent' }};
                      border-left:3px solid {{ $isActive ? 'var(--color-rose)' : 'transparent' }}">
                <div class="d-flex align-items-center gap-3 px-4 py-3"
                    ...
                    <div class="flex-fill" style="min-width:0">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            @if($unread > 0)
                            <span style="background:var(--color-rose);color:#fff;
            ...
            <div style="text-align:center;padding:40px 20px;color:var(--color-text-mid)">
                <p class="text-muted-sm">No conversations yet.</p>
    ...
    {{-- Message Thread --}}
    <div class="col-md-8">
        @if(isset($teacher) && isset($messages))
        <div class="educore-card" style="padding:0;overflow:hidden;display:flex;flex-direction:column">
            {{-- Header --}}
            <div class="px-4 py-3" style="border-bottom:1px solid var(--color-border);
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-muted-sm" style="font-size:12px">Teacher</div>
                    </div>
                </div>
            </div>
            ...
            <div style="padding:20px;min-height:300px;max-height:400px;overflow-y:auto"
                ...
                <div class="d-flex {{ $isMine ? 'justify-content-end' : 'justify-content-start' }} mb-3">
                    <div style="max-width:75%">
                        ...
                        <div style="padding:10px 14px;border-radius:12px;font-size:14px;
                                    background:{{ $isMine ? 'var(--color-rose)' : 'var(--color-bg)' }};
                ...
                <div style="text-align:center;padding:40px 0;color:var(--color-text-mid)">
                    <p class="text-muted-sm">No messages yet. Say hello!</p>
                </div>
                @endforelse
            </div>
            ...
            <div style="padding:16px;border-top:1px solid var(--color-border);background:var(--color-bg)">
                <form method="POST" action="{{ route('parent.messages.send') }}" id="sendForm">
                    @csrf
                    <input type="hidden" name="receiver_id" value="{{ $teacher->id }}">
                    <div class="d-flex gap-2">
                        <textarea name="body" id="messageBody" class="form-control" rows="2"
        ...
        (function () {
            ...
            const pollUrl  = "{{ route('parent.messages.poll', $teacher->id) }}";
            const myColor  = 'var(--color-rose)';
        ...
        <div class="educore-card" style="text-align:center;padding:80px 0;color:var(--color-text-mid)">
            ...
            <p class="fw-semibold">Select a conversation</p>
            <p class="text-muted-sm">or start a new one with a teacher</p>
        </div>
        @endif
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-semibold">New Message</h5>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select name="receiver_id" class="form-select" required>
                            ...
                            <option value="{{ $t->user_id }}">{{ $t->user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Regarding Child (Optional)</label>
